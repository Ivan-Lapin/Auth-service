...
func (ur *UserRepository) Save(user *domain.User) error {
	...
	if err != nil {
		ur.logger.Error("failed to save user", zap.String("Username", user.Username), zap.String("Email", user.Email), zap.String("Password", user.Password))
...
func (ur *UserRepository) FindUserByEmail(email string) (*domain.User, error) {
	...
	if errors.Is(err, sql.ErrNoRows) {
		ur.logger.Error("There are not user with that email", zap.String("Email", user.Email))
		return nil, apperrors.ErrUserAlreadyExist
	}

	if err != nil {
		ur.logger.Error("Failed to find user by email", zap.String("Email", user.Email))
